...
GifParser.prototype.setGlobalColorTable = function(colorTable){
  ...
  var tableSize = (colorTable.length >>> 0).toString(2).length+1;
  console.log("table size",tableSize)
  this.globalColorTable = new ColorTable();
  this.header.GLOBAL_COLOR_TABLE_SIZE = Math.max(4,tableSize);
...
GifParser.prototype.addFrame = function(img,options){
  ...
  var img = new ImageDescriptor(img,this,{left:options.left || 0 ,top:options.top || 0 ,width: options.width || this.header.width,height:options.height || this.header.height});
  ...
  if(options.crop){
    ...
    for(i = 0;i<imgs.length-1;i++){
      ...
      //  console.log("compare ",area.left,area.top,area.width,area.height)
        ...
        console.log(area.width==0 || area.height==0)
