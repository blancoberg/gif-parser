...
ImageDescriptor.prototype.compareTo = function(img){
  ...
  if(width == img.width && height == img.height){
    ...
    // right
    for(var i = data1.length-1;i>-1;i--){
      var id = (i%height) * width + Math.ceil(i/height)
    	if(data1[id] != data2[id]){
      	right = Math.ceil(i/height);
        break;
      }
    }

    //
    return {left:left,top:top,width:width - left-(width-right),height:height-top-(height-bottom)}
...
ImageDescriptor.prototype.convertCanvasToData = function(file){
  ...
  var imgData = file.getContext("2d").getImageData(0,0, this.width, this.height);
...
ImageDescriptor.prototype.encodeImageData = function(){
  ...
  this.blockData = LZW.encode(this.data,this.minimumCodeSize);
