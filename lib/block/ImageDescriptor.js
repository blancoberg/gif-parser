function ImageDescriptor(file,parser,options){
  ...
  if(file.type){
    ...
    this.hasLocalColorTable = options.localColorTable ? true : false;
    this.interlaced = this.packedBits[1] == "1" ? true : false;
    this.sortFlag = this.packedBits[2] == "1" ? true : false;
    this.reserved = this.packedBits.slice(3,4);
    this.colorTableSize = 0; // temporary

    if(this.hasLocalColorTable){
      //this.colorTable = this.generateColorTable(file);
    }else{
      this.colorTable = parser.globalColorTable;
    }
...
ImageDescriptor.prototype.compareTo = function(img){
  ...
  if(width == img.width && height == img.height){
    ...
    for(var i = data1.length-1;i>-1;i--){
    	if(data1[i] != data2[i]){
      	bottom = width - Math.ceil(i/width);
        break;
      }
    }
    ...
    for(var i = data1.length-1;i>-1;i--){
      var id = (i%height) * width + Math.floor(i/height)
    	if(data1[id] != data2[id]){
      	right = height - Math.ceil(i/height);
        break;
      }
    }
...
ImageDescriptor.prototype.findNearestColor = function(r,g,b,colorTable){
  ...
  for(var i = 0;i<colorTable.length;i++){
    var c = colorTable[i];
    var tempDist = Math.pow((c.r-r)*1,2) + Math.pow((c.g-g)*1,2) + Math.pow((c.b-b)*1,2);
    if(tempDist< dist){
      color = i;
      dist = tempDist;
    }
...
ImageDescriptor.prototype.convertCanvasToData = function(file){
  ...
  if(this.hasLocalColorTable){
    ...
    for(var i = 0;i<pixels.length;i+=4){
      var color = ((1 << 24) + (pixels[i] << 16) + (pixels[i+1] << 8) + pixels[i+2]);
      newColors[color] = newColors[color] == null ? 0 : newColors[color]+1;
    }
    var count = 0;
    //newColors.sort();
    var finalColors = [];
    for(var a in newColors){
      if(newColors[a] == 0){
        delete newColors[a];
      }else{

        count++;
        finalColors.push(Number(a));
      }
    ...
    var c = [];
    for(var i = 0;i<128;i++){
      c.push(((1 << 24) + (i*2 << 16) + (i*2 << 8) + i*2))
    }
    this.colorTable = new ColorTable(c);
    colors = this.colorTable.colors;
    this.colorTableSize = 9;
    console.log("colors",finalColors,count)
  }
